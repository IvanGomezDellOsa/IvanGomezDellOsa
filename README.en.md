...
The private investments announced in Argentina were scattered across news outlets, X accounts and official registries. This project automates collecting and publishing that information: every 72 hours, a scheduled GitHub Actions workflow gathers material from a range of sources and sweeps the web for investment mentions with Gemini and Search Grounding. A layered system decides what gets published: Gemini structures each mention, pgvector retrieves the closest records, and identity rules together with Jev (TypeSafe) resolve what is a real investment and what is already published under a different name. Publishing runs unattended, to the website and a Telegram channel, with a FastAPI API and a Next.js frontend with server-side rendering, on Vercel.
